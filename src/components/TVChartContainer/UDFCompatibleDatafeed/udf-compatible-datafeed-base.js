...
import { WebsocketUpdaterProvider } from './websocket-updater-provider';
...
var UDFCompatibleDatafeedBase = /** @class */ (function () {
    function UDFCompatibleDatafeedBase(datafeedURL, quotesProvider, requester, wsMode, updateFrequency) {
        if (wsMode === void 0) { wsMode = true; }
        ...
        if (wsMode) {
            this._dataUpdater = new WebsocketUpdaterProvider();
        }
        else {
            this._dataUpdater = new DataPulseProvider(this._historyProvider, updateFrequency);
        }
    ...
    UDFCompatibleDatafeedBase.prototype.getBars = function (symbolInfo, resolution, rangeStartDate, rangeEndDate, onResult, onError) {
        this._historyProvider.getBars(symbolInfo, resolution, rangeStartDate, rangeEndDate)
            ...
            var resolutionSeconds = "string" == typeof resolution ? getResolutionSeconds(resolution) : resolution;
            var noData = result.meta.noData;
            if (rangeEndDate + 60 * resolutionSeconds > Date.now() / 1000 && result.bars.length > 0 && !noData) {
                window.postMessage({
                    event: "gotBars",
                    symbol: symbolInfo.name,
                    lastBar: result.bars[result.bars.length - 1],
    ...
    UDFCompatibleDatafeedBase.prototype.subscribeBars = function (symbolInfo, resolution, onTick, listenerGuid, onResetCacheNeededCallback) {
        this._dataUpdater.subscribeBars(symbolInfo, resolution, onTick, listenerGuid, onResetCacheNeededCallback);
    };
    UDFCompatibleDatafeedBase.prototype.unsubscribeBars = function (listenerGuid) {
        this._dataUpdater.unsubscribeBars(listenerGuid);
    };
...
function getResolutionSeconds(resolution) {
    ...
    var time = resolution.slice(0, -1) || 1;
    var unit = resolution[resolution.length - 1];
    return TIME_FRAMES_UNIT_SECONDS[unit] * parseInt(time, 10);
}
