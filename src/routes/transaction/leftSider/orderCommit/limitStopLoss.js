    ...
    orderButtonClick = (type) => {
        ...
        if (this.checkFlag) {
            dispatch({
                ...
                payload: {
                    addVisible: { addVisibleLimitStop: true },
                    title: $("确认你的委托"),
        ...
        return (
            <div className="orderLimitPrice">
                        ...
                        <Row className='rowFlex' style={{ marginBottom: 5 }}>
                            ...
                            <Col className='rowItemSecond'>
                                <Tooltip mouseLeaveDelay={0} placement="right" title={tooltipShow(translationParameters([symbolCurrent], $('触发后平仓提示信息')) + '<br/><br/>' + $('使用它以你的平仓委托只会减少你的仓位。'))}>
                                    {/* <Checkbox style={{ color: "#333333", fontSize: 12 }}
                        ...
                        <Row className='rowFlex' style={{ marginBottom: 5 }}>
                            <Col className='rowItemFirst'>
                                <Tooltip mouseLeaveDelay={0} placement="right" title={tooltipShow($("被动委托不会立即在市场成交。使用该类型来确保获得做市返佣。如果委托会立刻与已有委托成交，那么该委托将被取消。"))}>
                                    <input disabled={this.state.displayQtyFlag || this.state.timeInForce !== "GoodTillCancel"} className='checkbox_class' id={"ParticipateDoNotInitiateLoss"} onChange={(e) => this.onChangeCheckBox(e, "ParticipateDoNotInitiate")} type="checkbox"></input>
                                    <label style={{ cursor: "help" }} htmlFor={"ParticipateDoNotInitiateLoss"}>{$('被动委托')}</label>
                            ...
                            <Col className='rowItemSecond'>
                                ...
                                onChange={(e) => this.onChangeCheckBox(e, "displayQtyFlag")}><span style={{ cursor: 'help' }}>{$('隐藏')}</span></Checkbox> */}
                                    <label style={{ cursor: "help" }} htmlFor={"displayQtyFlagLoss"}>{$('隐藏')}</label>
                                    <input disabled={this.state.ParticipateDoNotInitiate} className='checkbox_class' id={"displayQtyFlagLoss"} onChange={(e) => this.onChangeCheckBox(e, "displayQtyFlag")} type="checkbox"></input>
                                </Tooltip>
                            </Col>
                        </Row>
                ...
                <ButtonPriceLimit
                    ...
                    height="70px"
                    marketStop="limitStopLoss"
                    stopBuyFlag={false}
                    stopSellFlag={true}
                    orderButtonClick={this.orderButtonClick} />
                <AddModal closeFlag={this.state.closeFlag} timeInForce={this.state.touchType} ordTypeTitle="限价止损" ordTypeTitleFlag="addVisibleLimitStop" checkFlagChange={this.checkFlagChange} orderCommit={this.orderCommit} />
            </div >
        )
    }
}
