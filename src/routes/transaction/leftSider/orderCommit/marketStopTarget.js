...
let $ = getLanguageData;
export class MarketStopTarget extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataSource: {
    ...
    componentDidMount = () => {
        ...
        let dataSource = this.state.dataSource;
        try {
            dataSource.stopPx = (lastPrice * 1 - _tickSize).toFixed(getTickLength(_tickSize));
        } catch (error) {
    ...
    orderButtonClick = (type) => {
        ...
        if (this.checkFlag) {
            dispatch({
                ...
                payload: {
                    addVisible: { addVisibleMarketTarget: true },
                    title: $("确认你的委托"),
    ...
    render() {
        const { symbolCurrent, tickSize, showPlusMult } = this.props;
        let _tickSize = tickSize || 0.5;
        ...
        return (
            <div className="orderLimitPrice">
                ...
                <ButtonPriceLimit
                    textBuy="止盈买入"
                    textSell="止盈卖出"
                    orderQty={this.state.dataSource.orderQty || 1} stopPx={this.state.dataSource.stopPx || 0}
                    flag={flag}
                    touchType={this.state.timeInForce}
                    orderType="marketStopTarget"
                    height="70px"
                    marketStop="market"
                    stopBuyFlag={true}
                    stopSellFlag={true}
                    orderButtonClick={this.orderButtonClick} />
                <AddModal closeFlag={this.state.closeFlag} timeInForce={this.state.timeInForce} ordTypeTitle="市价止盈" ordTypeTitleFlag="addVisibleMarketTarget" checkFlagChange={this.checkFlagChange} orderCommit={this.orderCommit} />
            </div >
        )
    }
}
